...
/**
 * ローカル座標をワールド座標に変換する
 * (position + rotation の Euler 変換を適用)
 */
function localToWorld(
    ...
    return local.clone().applyQuaternion(quat).add(
        new THREE.Vector3(position[0], position[1], position[2]),
    );
}

type BranchSurfacePoint = {
    position: Vec3;
    direction: Vec3;
};

/**
 * 全枝 (主枝 + サブブランチ) のカーブ終端部を歩いて配置点を収集する
 * 各枝カーブの t=0.8〜1.0 に沿った点 + 接線方向をワールド座標で返す
 */
function collectBranchSurfacePoints(
    branches: Branch[],
    trunkHeight: number,
    samplesPerBranch: number,
): BranchSurfacePoint[] {
    const pts: BranchSurfacePoint[] = [];
    const total = Math.min(branches.length, MAX_MAIN_BRANCHES);

    for (let i = 0; i < total; i++) {
        const branch = branches[i];
        const transform = computeBranchTransform(branch, trunkHeight, i, total);
        const scale = branchLengthScale(i, total);
        const scaledLength = branch.length * scale;
        const curve = createBranchCurve(scaledLength, branch.seed);

        // 主枝の終端部 (t=0.8〜1.0) をサンプリング
        for (let s = 0; s < samplesPerBranch; s++) {
            const t = 0.8 + (s / Math.max(samplesPerBranch - 1, 1)) * 0.2;
            const localPt = curve.getPointAt(t);
            const localTan = curve.getTangentAt(t).normalize();
            const worldPt = localToWorld(localPt, transform.position, transform.rotation);
            const worldTan = localToWorld(
                localTan.clone().add(localPt),
                transform.position,
                transform.rotation,
            ).sub(worldPt).normalize();
            pts.push({
                position: [worldPt.x, worldPt.y, worldPt.z],
                direction: [worldTan.x, worldTan.y, worldTan.z],
            });
        }

        // サブブランチの終端部も収集
        const subs = computeSubBranches(scaledLength, branch.depth, branch.seed);
        for (const sub of subs) {
            const attachPt = curve.getPointAt(sub.attachT);
            const attachWorld = localToWorld(attachPt, transform.position, transform.rotation);
            const subQuat = new THREE.Quaternion().setFromEuler(
                new THREE.Euler(0, sub.yRotation, sub.angle * DEG_TO_RAD),
            );

            const subCurve = createBranchCurve(sub.length, sub.seed);
            const subSamples = Math.max(Math.floor(samplesPerBranch / 2), 2);
            for (let s = 0; s < subSamples; s++) {
                const t = 0.8 + (s / Math.max(subSamples - 1, 1)) * 0.2;
                const subLocal = subCurve.getPointAt(t);
                const subTan = subCurve.getTangentAt(t).normalize();
                const subWorldPt = subLocal.clone().applyQuaternion(subQuat).add(attachWorld);
                const subWorldTan = subTan.clone().applyQuaternion(subQuat).normalize();
                pts.push({
                    position: [subWorldPt.x, subWorldPt.y, subWorldPt.z],
                    direction: [subWorldTan.x, subWorldTan.y, subWorldTan.z],
                });
            }
        }
    }
...
/**
 * 枝表面の点から枝の延長方向に扇形に配置する
 *
 * - 枝方向 (dir) に前進するのが主軸
 * - 前進距離に比例して横に広がる (先端から扇形)
 * - 根元付近は密集、先端に向かって広がる
 */
function placeOnBranchSurface(
    surfacePoints: BranchSurfacePoint[],
    count: number,
    spread: number,
    rng: () => number,
    weights: number[],
    ...
    const placeAtPoint = (pt: BranchSurfacePoint, n: number) => {
        const dir = new THREE.Vector3(pt.direction[0], pt.direction[1], pt.direction[2]);
        ...
        for (let j = 0; j < n; j++) {
            // 枝方向への前進 (主軸): ガウシアンで根元寄りに密集
            const u1 = Math.max(rng(), 0.0001);
            const u2 = rng();
            const gauss = Math.abs(Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2));
            const forward = spread * gauss * 0.8;

            // 横方向の広がり: 前進距離に比例して扇形に広がる
            const fanWidth = forward * 0.6 + spread * 0.05;
            const angle = rng() * Math.PI * 2;
            const lateral = fanWidth * Math.sqrt(rng());

            const px = pt.position[0] + dir.x * forward
                + perp1.x * lateral * Math.cos(angle)
                + perp2.x * lateral * Math.sin(angle);
            const py = pt.position[1] + dir.y * forward
                + perp1.y * lateral * Math.cos(angle)
                + perp2.y * lateral * Math.sin(angle);
            const pz = pt.position[2] + dir.z * forward
                + perp1.z * lateral * Math.cos(angle)
                + perp2.z * lateral * Math.sin(angle);
            positions.push([px, py, pz]);
        }
    };

    let remaining = count;
    for (let i = 0; i < surfacePoints.length && remaining > 0; i++) {
        const share = Math.round((weights[i] / totalWeight) * count);
        const n = Math.min(Math.max(share, 0), remaining);
        if (n === 0) continue;
        placeAtPoint(surfacePoints[i], n);
        remaining -= n;
    }
    if (remaining > 0 && surfacePoints.length > 0) {
        const maxIdx = weights.indexOf(Math.max(...weights));
        placeAtPoint(surfacePoints[maxIdx], remaining);
    }
...
/**
 * 2〜3 箇所のメインクラスターに集中する重みを計算する
 *
 * 1. 最も高い枝先 → メインクラスター (最大の重み)
 * 2. 最も横に広がった枝先 → サブクラスター
 * 3. その他 → 控えめ
 */
function computeClusterWeights(
    pts: BranchSurfacePoint[],
    rng: () => number,
): number[] {
    ...
    for (let i = 1; i < pts.length; i++) {
        if (pts[i].position[1] > pts[topIdx].position[1]) topIdx = i;
    }

    // 最も横に広がった点 (xz 距離が最大) を見つける
    let sideIdx = 0;
    let maxXZ = 0;
    for (let i = 0; i < pts.length; i++) {
        const [x, , z] = pts[i].position;
        const dist = x * x + z * z;
        if (dist > maxXZ) { maxXZ = dist; sideIdx = i; }
    }
    // top と side が同じなら次に横に広い点を探す
    if (sideIdx === topIdx) {
        let secondMaxXZ = 0;
        for (let i = 0; i < pts.length; i++) {
            ...
            const [x, , z] = pts[i].position;
            const dist = x * x + z * z;
            if (dist > secondMaxXZ) { secondMaxXZ = dist; sideIdx = i; }
        }
    }

    return pts.map((_, i) => {
        if (i === topIdx) return 5.0 + rng() * 1.0;       // 上: メイン
        if (i === sideIdx) return 3.0 + rng() * 1.0;      // 横: サブ
        return 0.3 + rng() * 0.4;                          // その他: 控えめ
    });
}
 ...
 * 枝カーブの終端部に密着して配置
 */
export function computeLeafPositions(
    count: number,
    branches: Branch[],
    trunkHeight: number,
): Vec3[] {
    ...
    const rng = seededRandom(count * 7919 + branches.length * 104729);
    ...
    const pts = collectBranchSurfacePoints(branches, trunkHeight, 5);
    const weights = computeClusterWeights(pts, rng);
    const totalWeight = weights.reduce((a: number, b: number) => a + b, 0);

    return placeOnBranchSurface(pts, count, 0.06, rng, weights, totalWeight);
}

/**
 * 花のインスタンス配置位置を計算する
 * 枝カーブの終端部に密着、メインクラスターに集中
 */
export function computeFlowerPositions(
    ...
    const rng = seededRandom(count * 6271 + branches.length * 97531);
    ...
    const pts = collectBranchSurfacePoints(branches, trunkHeight, 4);
    const weights = computeClusterWeights(pts, rng);
    const totalWeight = weights.reduce((a: number, b: number) => a + b, 0);

    return placeOnBranchSurface(pts, count, 0.05, rng, weights, totalWeight);
}
