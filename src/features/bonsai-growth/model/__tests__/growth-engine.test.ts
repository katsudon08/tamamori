...
const RULES: GrowthRule[] = [
    ...
    { id: '4', stage: 'branching', min_messages: 30, min_reactions: 15, min_thanks: 3, sort_order: 3 },
    { id: '5', stage: 'leafy', min_messages: 60, min_reactions: 30, min_thanks: 10, sort_order: 4 },
    { id: '6', stage: 'budding', min_messages: 100, min_reactions: 50, min_thanks: 20, sort_order: 5 },
    { id: '7', stage: 'flowering', min_messages: 150, min_reactions: 80, min_thanks: 35, sort_order: 6 },
    { id: '8', stage: 'full_bloom', min_messages: 250, min_reactions: 120, min_thanks: 60, sort_order: 7 },
];

describe('determineStage', () => {
    test('(0, 0, 0) → seed', () => {
        expect(determineStage({ totalMessages: 0, totalReactions: 0, totalThanks: 0 }, RULES))
            .toBe('seed');
    });

    test('(5, 0, 0) → sprout', () => {
        expect(determineStage({ totalMessages: 5, totalReactions: 0, totalThanks: 0 }, RULES))
            .toBe('sprout');
    });

    test('(15, 5, 0) → young', () => {
        expect(determineStage({ totalMessages: 15, totalReactions: 5, totalThanks: 0 }, RULES))
            .toBe('young');
    });

    test('(30, 15, 3) → branching', () => {
        expect(determineStage({ totalMessages: 30, totalReactions: 15, totalThanks: 3 }, RULES))
            .toBe('branching');
    });

    test('(250, 120, 60) → full_bloom', () => {
        expect(determineStage({ totalMessages: 250, totalReactions: 120, totalThanks: 60 }, RULES))
            .toBe('full_bloom');
    });

    test('一つでも閾値未達なら前ステージ: (150, 80, 34) → budding (thanks=34 < 35)', () => {
        expect(determineStage({ totalMessages: 150, totalReactions: 80, totalThanks: 34 }, RULES))
            .toBe('budding');
    });
});
